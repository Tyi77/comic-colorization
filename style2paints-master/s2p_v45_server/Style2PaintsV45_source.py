...
def merge_color_blocks(data1, data2, distance_threshold=0.001):
    # 將字串轉換為 Python 列表
    blocks1 = json.loads(data1)
    blocks2 = json.loads(data2)
    
    # 將 blocks1 轉換為字典以便快速查找
    merged_blocks = {tuple(block[:2]): block for block in blocks1}
    
    for block2 in blocks2:
        x2, y2 = block2[:2]
        replace_key = None
        
        # 檢查與 blocks1 中的任何色塊是否接近
        for key in merged_blocks:
            x1, y1 = key
        ...
        # 如果接近，更新該色塊；否則新增
        if replace_key:
            merged_blocks[replace_key] = block2
        else:
            merged_blocks[(x2, y2)] = block2
